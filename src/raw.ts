...
export type MimeType = "application/pdf" | "application/epub+zip";
...
export interface RawRemarkableApi {
   ...
   * even if the backend schema version is version 4.
   *
   ...
   * @param opts - optional options for the upload, such as the parent id
...
export class RawRemarkable implements RawRemarkableApi {
  ...
  async uploadFile(
    ...
    const meta = fromByteArray(
      enc.encode(JSON.stringify({ file_name: visibleName, parent })),
    );
    const resp = await this.#authedFetch(
      "POST",
